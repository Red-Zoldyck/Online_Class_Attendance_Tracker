...
- **Location**: `templates/classes/section_course_detail.html`
- **Format**: 12-hour format (HH:MM:SS AM/PM) with full date
- **Update**: Every 1000ms (1 second)
- **Styling**: Transparent background with border, embedded in page
- **JavaScript**: Uses `Date` API with DOMContentLoaded event listener

### CSV Student Import
- **Location**: `apps/users/views.py` - `AdminStudentListView`
- **Features**:
  - Case-insensitive column matching
  - Flexible column name matching: 'student-ID', 'student_number', 'student_id', 'sr_code'
  - Name parsing: Handles "LASTNAME, FIRSTNAME MIDDLE" or "FIRST LAST" formats
  - Title-casing: Converts names to proper case (e.g., "Maria Danica")
  - Duplicate prevention: Checks `student_number` first
  - Section/term assignment during import

### Attendance Reports Dashboard
- **Location**: `apps/reports/views.py` - `ReportDashboardView`
- **Features**:
  - Role-based filtering (admin sees all, instructor sees own, student sees own)
  - Class/section dropdown selector
  - Date range filtering
  - Status code mapping: P (Present), A (Absent), L (Late), E (Excused)
  - Summary statistics and detailed records
  - Print-optimized CSS hiding filters
- **Template**: `templates/reports/attendance_dashboard.html`

### Schedule Enforcement
- **Attendance Window**: Opens at 6 AM
- **Per-Student Lock**: 4 hours after marking
- **Report Deadline**: 8 hours after lock
- **Implementation**: `apps/classes/models.py` uses timestamps and comparisons
- **Display**: Shows lock status and remaining time on attendance page

### Enrollment Management
- **Unique Constraint**: (student, course, term) enforced at database level
- **Auto-Enrollment**: `_auto_enroll_for_section_course()` and `_auto_enroll_for_student_section()`
- **Race Condition Handling**: Try/except blocks for concurrent requests
- **Duplicate Prevention**: Uses `get_or_create()` with correct lookup keys

### Admin Account Registration
- **Code**: `@dm|n@2o2G!`
- **Validation**: `apps/users/serializers.py` - `UserRegistrationSerializer.validate()`
- **Role Assignment**: Admin role assigned upon successful validation
...
When developing locally or setting up the system, use the following credentials for admin registration:

**Admin Registration Code**: `@dm|n@2o2G!`

This code is validated in `/apps/users/serializers.py` during registration. The code must be provided when selecting "Admin" as the role during user registration.

**How to register as admin**:
1. Navigate to registration page
2. Select "Admin" from role dropdown
3. Enter the admin code: `@dm|n@2o2G!`
4. Complete registration

For changing the admin code, update the validation string in:
- File: `/apps/users/serializers.py`
- Line: ~215 (in the `validate` method)
- Find: `if code != "@dm|n@2o2G!":`
