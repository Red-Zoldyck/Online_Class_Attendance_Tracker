...
- **Database**: MySQL 8.0+ with mysql-connector-python 8.2.0
...
The application includes a comprehensive web interface with the following features:

#### Real-time Digital Clock
- **Format**: 12-hour format (HH:MM:SS AM/PM) with full date display (e.g., Friday, January 3, 2026)
- **Location**: Embedded at the top of the section course detail page
- **Styling**: Transparent background with border, synced to local device time
- **Update Frequency**: Updates every second

#### Attendance Marking & Schedule Enforcement
- **Attendance Window**: Opens at 6 AM each day
- **Per-Student Lock**: 4 hours after marking attendance (prevents re-marking)
- **Report Deadline**: 8 hours after the per-student lock expires
- **Status Options**: Present (P), Absent (A), Late (L), Excused (E)
- **UI Status Display**: Shows locked/unlocked state and remaining time

#### Attendance Reports Dashboard
- **Access**: Via "Reports" link in main navigation
- **Filtering**: By section/course and date range
- **Summary View**: Statistics grouped by section and subject
- **Detailed View**: Student name, number, date, and status code
- **Export**: Print-optimized layout for report generation
- **Status Codes**: P=Present, A=Absent, L=Late, E=Excused

#### Student Management Portal
- **CSV Import**: Case-insensitive column matching (e.g., 'student-ID', 'student_number', 'Student ID')
- **Name Parsing**: Flexible formats (LASTNAME, FIRSTNAME MIDDLE or FIRST LAST)
- **Data Processing**: Title-casing for proper storage (e.g., "Maria Danica")
- **Filtering**: By section and term
- **Sorting**: Multi-field sorting (last_name, first_name, email, student_number, section, enrollments)
- **Duplicate Prevention**: Checks student_number first before creating new record

#### Navigation Integration
- **Reports Link**: Available on all main pages (dashboard, classes list, admin panels)
- **Unified Navigation**: Consistent menu structure across web interface

### Enrollment & Course Management
- **Unique Constraints**: (student, course, term) enforced at database level
- **Auto-Enrollment**: Automatic enrollment when student joins section
- **Race Condition Handling**: Try/except blocks for concurrent requests
- **Section Syncing**: Auto-sync enrollments across all section courses
  - Export reports with print functionality
  - Status codes: P (Present), A (Absent), L (Late), E (Excused)
- **Student Management**: 
  - Import students from CSV (case-insensitive)
  - Assign sections and terms
  - Manage enrollments
- **Schedule Management**: Assign schedules to section courses
- **Navigation**: Unified navigation across all pages (Dashboard, Classes, Reports, Admin panels)
